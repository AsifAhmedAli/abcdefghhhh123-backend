...
router.get("/getRecommendations", function (req, res, next) {
  ...
  let prefId = req.query.prefId;
  connection.query(
    "select clothing.Clothing_type,clothing.clothing_image_path, recommended.preference_id from clothing join recommended on clothing.clothing_id=recommended.clothing_id  where apparent_temp_range_start < ? and apparent_temp_range_end > ? and recommended.preference_id=?;",[apparentTemp,apparentTemp,prefId],
    function (error, results, fields) {
