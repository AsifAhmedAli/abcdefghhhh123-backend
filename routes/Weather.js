...
router.get('/getWeatherDataWeekly', function(req, res, next) {
    ...
    // create a moment object with the desired date and time with offset as parameter
    const dateOff = moment.tz(date.format(), tz);
    const amData = dateOff.clone().add(6, 'hours').utc().format();
    console.log(amData);
    const pmData = dateOff.clone().add(18, 'hours').utc().format();
    console.log(pmData);
    // convert to UTC
    const date_utc_start = dateOff.utc();
    // print the UTC date for the start datetime
     ...
     function(error, results, fields) {
        if (error) throw error;
            ...
            for (let index = 0; index < results.length; index++) {
                    ...
                    if(element<=30){
                        results[index].forecast = 'Sunny';
                    }
                    else if(element > 30 && element < 70){
                        results[index].forecast = 'Sunny&Cloudy';
                    }
                    else if(element >= 70){
                        results[index].forecast = 'Cloudy';
                    }
                }
            res.send(results);
        }
